...
async function main() {
  ...
  if (!ws) { console.error("usage: verify_executor.mjs <workspace-dir>"); process.exitCode = 1; return; }
  ...
  try {
    ...
    const result = await adapter.execute({ task, text: task, plan: { acceptance: [] } });
    ...
    if (!(exists && content === nonce)) process.exitCode = 1;
  } finally {
